...
function useStage4Dimensions() {
  const { width: W, height: H } = useWindowDimensions();
  const CANVAS_PAD = Math.round(W * 0.05);
  const MAX_W = W - CANVAS_PAD * 2;
  const MAX_H = Math.round(H * 0.52);
  const SIZE = Math.min(MAX_W, MAX_H);
  const CANVAS_W = SIZE;
  const CANVAS_H = SIZE;
  const LETTER_SIZE = Math.min(CANVAS_W, CANVAS_H) * 0.85;
  const LETTER_OX = (CANVAS_W - LETTER_SIZE) / 2;
  const LETTER_OY = (CANVAS_H - LETTER_SIZE) / 2;

  return { W, H, CANVAS_PAD, CANVAS_W, CANVAS_H, LETTER_SIZE, LETTER_OX, LETTER_OY };
}

/** Map a unit-square point to canvas pixel coords via the letter box */
function letterToCanvas(
...
function buildLetterSvgPath(
  ...
  const p = pts.map((pt) => letterToCanvas(pt, LETTER_OX, LETTER_OY, LETTER_SIZE));
...
function SparkleGuide({
  ...
  return (
    ...
    >
      ...
      >
        <Text style={{ fontSize: isDemo ? 28 : 24 }}>
          {isDemo ? "✨" : "👆"}
        </Text>
      </Animated.View>
    </View>
...
function LetterPopup({
  ...
  return (
    <View style={s.popupOverlay}>
      <Animated.View
        style={[s.popupCard, { transform: [{ scale: scaleAnim }] }]}
      >
...
function DrawingCanvas({
  ...
  const { W, H, CANVAS_PAD, CANVAS_W, CANVAS_H, LETTER_SIZE, LETTER_OX, LETTER_OY } = useStage4Dimensions();
  ...
  const pendingEvalRef = useRef<(() => void) | null>(null);
  ...
  const startPauseTimer = useCallback(
    (evalFn: () => void) => {
      clearPauseTimer();
      pendingEvalRef.current = evalFn;
      setIsPaused(true);
      let remaining = 30;
      setPauseCountdown(remaining);
      countdownIntervalRef.current = setInterval(() => {
        remaining -= 1;
        setPauseCountdown(Math.max(0, remaining));
      }, 1000);
  ...
  const guideSvgPath = useMemo(
    () => buildLetterSvgPath(level.guidePath, level.smooth, LETTER_OX, LETTER_OY, LETTER_SIZE),
  ...
  const runDemo = useCallback(() => {
    ...
    demoTimerRef.current = setInterval(() => {
      ...
      const mapped = letterToCanvas(pt, LETTER_OX, LETTER_OY, LETTER_SIZE);
  ...
  const effectiveCheckpoints = useMemo(() => {
    const pts = level.keyPoints.map((idx) =>
      letterToCanvas(level.guidePath[idx], LETTER_OX, LETTER_OY, LETTER_SIZE),
    );
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt) => {
        ...
        if (curCp < effectiveCheckpoints.length) {
          ...
          if (d <= level.tolerance * 1.4) {
            ...
            if (
              newCp < effectiveCheckpoints.length &&
              phaseRef.current === "guided"
            ) {
      ...
      onPanResponderRelease: () => {
        ...
        allDrawnPointsRef.current = allDrawnPointsRef.current.concat(
          drawnPointsRef.current,
        );
        ...
        return (
          ...
          >
            <Text
              style={[
                s.keyDotText,
                (isStart || isReached) && { color: PAL.white },
              ]}
            >
  ...
  return (
    <View style={s.canvasOuter}>
      ...
      {/* Pause overlay — 30s inactivity countdown */}
      <PauseOverlay
        visible={isPaused}
      ...
      {!completed && accuracy !== null && feedback && !isDrawing && (
        <View style={s.accuracyOverlay}>
          ...
          >
            ...
            <Text style={[s.controlLabel, { color: PAL.textDark }]}>
              Try Again
            </Text>
          </TouchableOpacity>
        </View>
...
function Stage4Gameplay() {
  ...
  const [progress, setProgress] = useState<
    Record<number, { starsEarned: number; completed: boolean }>
  >(() => {
    const p: Record<number, { starsEarned: number; completed: boolean }> = {};
    LETTERS.forEach((l) => {
      p[l.id] = { starsEarned: 0, completed: false };
    });
    return p;
  });
  ...
  const highestUnlocked = Math.max(
    1,
    ...LETTERS.filter((l) => progress[l.id].completed).map((l) => l.id + 1),
  );
  ...
  const handleComplete = (stars: number) => {
    setProgress((prev) => ({
      ...prev,
      [currentLetterId]: {
        starsEarned: Math.max(prev[currentLetterId].starsEarned, stars),
        completed: true,
      },
    }));
    setShowPopup(true);
      ...
      <View style={s.summaryRoot}>
        ...
        <View style={s.summaryCard}>
          ...
          <View style={s.summaryStarRow}>
            <Text
              style={{ fontSize: 32, fontWeight: "900", color: PAL.textDark }}
            >
              {totalStars}
            </Text>
          ...
          {LETTERS.map((level) => (
            <View key={level.id} style={s.summaryRow}>
              ...
              <Text style={s.summaryRowName}>
                {level.letter} — {level.name}
              </Text>
              <View style={{ flexDirection: "row" }}>
                {[1, 2, 3].map((i) => (
                  <Text key={i} style={{ fontSize: 14 }}>
                    {i <= progress[level.id].starsEarned ? "⭐" : "☆"}
                  </Text>
                ))}
              </View>
            </View>
    ...
    return (
      <View style={s.playRoot}>
        ...
        <View style={s.playHeader}>
          ...
          <Text style={s.playTitle}>
            {level.emoji} {level.letter} — {level.name}
          </Text>
  ...
  return (
    <View style={s.selectRoot}>
      ...
      <ScrollView contentContainerStyle={s.pathScroll}>
        {LETTERS.map((level) => {
          const unlocked =
            progress[level.id].completed || level.id <= highestUnlocked;
          const isNext =
            level.id === highestUnlocked && !progress[level.id].completed;
          return (
            <TouchableOpacity
              key={level.id}
              onPress={() => {
            ...
            >
              ...
              >
                ...
                {progress[level.id].completed &&
                  progress[level.id].starsEarned > 0 && (
                    <View style={s.bubbleStars}>
                      {Array.from(
                        { length: Math.min(progress[level.id].starsEarned, 3) },
                        (_, i) => (
              ...
              <Text
                style={[s.bubbleLabel, !unlocked && { color: PAL.lockGray }]}
              >
                {level.name}
              </Text>
      ...
      <View style={s.selectFooter}>
        <Text style={s.selectFooterText}>
          🐵 Help Milo restore the jungle map!
        </Text>
      </View>
    </View>
  );
...
const s = StyleSheet.create({
  ...
  canvasOuter: { flex: 1, paddingTop: 8 },
  canvasBg: {
    borderRadius: 20,
